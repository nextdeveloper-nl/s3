...
Tables defined here are prefixed with `s3_` or `iam_` to avoid collisions with existing schema.
...
One row per S3 bucket. Owned by an `s3_account`, physically residing on an `s3_server`. Orchestration owns the intent fields (`replication_factor`, `lifecycle_rules`). `storaged` writes the observed state fields (`object_count`, `size_bytes`, `replica_health`) via telemetry.
...
| `s3_buckets` | `name`, `replication_factor`, `lifecycle_rules` |
