<?php

namespace NextDeveloper\S3\Http\Transformers;

use Illuminate\Support\Facades\Cache;
use NextDeveloper\Commons\Common\Cache\CacheHelper;
use NextDeveloper\S3\Database\Models\BucketsPerspective;
use NextDeveloper\Commons\Http\Transformers\AbstractTransformer;
use NextDeveloper\S3\Http\Transformers\AbstractTransformers\AbstractBucketsPerspectiveTransformer;

/**
 * Class BucketsPerspectiveTransformer. This class is being used to manipulate the data we are serving to the customer
 *
 * @package NextDeveloper\S3\Http\Transformers
 */
class BucketsPerspectiveTransformer extends AbstractBucketsPerspectiveTransformer
{

    /**
     * @param BucketsPerspective $model
     *
     * @return array
     */
    public function transform(BucketsPerspective $model)
    {
        $transformed = Cache::get(
            CacheHelper::getKey('BucketsPerspective', $model->uuid, 'Transformed')
        );

        if($transformed) {
            return $transformed;
        }

        $transformed = parent::transform($model);

        Cache::set(
            CacheHelper::getKey('BucketsPerspective', $model->uuid, 'Transformed'),
            $transformed
        );

        return $transformed;
    }
}
