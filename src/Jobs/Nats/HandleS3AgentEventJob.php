...
/**
 * Dispatched by NatsListenCommand for every message received on agent.s3.*.evt.
 * Replaces the old S3AgentService::handle() dispatched via the main app's
 * App\Jobs\Nats\HandleS3AgentEventJob.
 *
 * Protocol: see docs/agent/seaweed-nats-contract.md §C
 */
class HandleS3AgentEventJob extends AbstractAgentEventJob
{
    // Dedicated queue — telemetry arrives every ~15-30s per agent and must not
    // share a worker with unrelated platform traffic on the default queue.
    public $queue = 's3-agent-events';

    private bool $wasPending = false;

    protected function resolveAgentModel(string $agentUuid)
    {
        return Servers::withoutGlobalScopes()->where('uuid', $agentUuid)->first();
    }
     ...
     *
     * Exception: a failed command result only proves the agent is reachable, not
     * that it applied anything correctly - don't flip agent_status to 'connected'
     * off the back of a failure (see test_failed_result_does_not_mark_server_connected).
     */
    protected function onAnyEvent($model, string $type, array $payload): void
    {
        if ($type === 'result' && ($payload['status'] ?? null) === 'failed') {
            $model->update(['agent_last_seen_at' => now()]);
            return;
        }

        $this->wasPending = $model->agent_status === 'pending';
     ...
     * are already refreshed unconditionally in onAnyEvent().
     */
    protected function updateHeartbeat($model, array $payload): void
    {
        if (!empty($payload['version']) && $payload['version'] !== $model->agent_version) {
            $model->update(['agent_version' => $payload['version']]);
        }

        // First heartbeat from a newly-provisioned server: send the full desired state.
        if ($this->wasPending) {
            Log::info('[HandleS3AgentEventJob] Pending server connected — dispatching full_sync', [
                'server_uuid' => $model->uuid,
            ]);
            S3AgentCommandService::fullSync($model->uuid);
        }
    }
    ...
    {
        match ($type) {
            'telemetry'    => $this->handleTelemetry($model, $payload),
            's3_telemetry' => $this->handleS3Telemetry($model, $payload),
            's3_audit'     => $this->handleS3Audit($model, $payload),
            'alert'        => $this->handleAlert($model, $payload),
            default        => Log::warning('[HandleS3AgentEventJob] Unknown message type', [
                'type'       => $type,
    ...
    /**
     * Mark the corresponding server healthy/failed and process full_sync/bucket.stats
     * output. Runs after AbstractAgentEventJob has already attempted to close out
     * the event_agent_commands row via AgentCommandsService::completeFromResult() -
     * $command may be null (e.g. a result with no matching dispatched command);
     * these side effects only depend on the payload itself, so that's fine here.
     */
    protected function onCommandResult($model, ?AgentCommands $command, array $payload): void
    {
        ...
        if ($status === 'completed') {
            ...
            if (array_key_exists('buckets_created', $output)) {
                Log::info('[HandleS3AgentEventJob] full_sync completed', [
            ...
            if (isset($output['buckets']) && is_array($output['buckets'])) {
                $this->updateBucketStatsFromTelemetry($server, $output['buckets']);
            }

            if (!empty($output['traffic'])) {
                $this->handleTrafficDeltas($server, $output['traffic']);
            }
     ...
     */
    private function handleS3Telemetry(Servers $server, array $payload): void
    {
        // Store raw component health from the agent (master, volume, filer, s3, etc.).
        // agent_status/agent_last_seen_at are already refreshed unconditionally in onAnyEvent().
        ...
        $this->updateBucketStatsFromTelemetry($server, $payload['buckets'] ?? []);

        // Accumulate per-bucket traffic deltas into the monthly bandwidth table
        $this->handleTrafficDeltas($server, $payload['traffic'] ?? []);
    }
     ...
     */
    private function handleTelemetry(Servers $server, array $payload): void
    {
        // agent_status/agent_last_seen_at are already refreshed unconditionally in onAnyEvent().
     ...
     */
    private function updateBucketStatsFromTelemetry(Servers $server, array $buckets): void
    {
        if (empty($buckets)) {
            return;
        }
        ...
        foreach ($buckets as $stats) {
            ...
            if (!$bucket) {
                Log::debug('[HandleS3AgentEventJob] Telemetry bucket not found in DB — skipping', [
                    'server_uuid' => $server->uuid,
                    'bucket_name' => $name,
                ]);
                continue;
        ...
        foreach (array_keys($touchedAccountIds) as $accountId) {
            $this->syncAccountStorageFromBuckets((int) $accountId);
        }

        Log::info('[HandleS3AgentEventJob] Bucket stats updated from telemetry', [
            'server_uuid'  => $server->uuid,
     ...
     */
    private function syncAccountStorageFromBuckets(int $accountId): void
    {
     ...
     */
    private function handleTrafficDeltas(Servers $server, array $traffic): void
    {
        if (empty($traffic)) {
            return;
        }
        ...
        foreach ($traffic as $delta) {
            ...
            if (!$bucket) {
                Log::debug('[HandleS3AgentEventJob] Traffic delta bucket not found — skipping', [
                    'server_uuid' => $server->uuid,
        ...
        Log::info('[HandleS3AgentEventJob] Traffic deltas accumulated', [
            'server_uuid'  => $server->uuid,
     ...
     */
    private function handleAlert(Servers $server, array $payload): void
    {
        $code = $payload['code'] ?? 'UNKNOWN';

        Log::warning('[HandleS3AgentEventJob] Agent alert', [
            'server_uuid' => $server->uuid,
     ...
     */
    private function handleS3Audit(Servers $server, array $payload): void
    {
        ...
        foreach ($events as $event) {
            ...
            if (!$bucket) {
                Log::debug('[HandleS3AgentEventJob] object_event bucket not found — skipping', [
                    'server_uuid' => $server->uuid,
                    'bucket_name' => $bucketName,
                ]);
                continue;
        ...
        Log::info('[HandleS3AgentEventJob] Object events logged', [
            'server_uuid' => $server->uuid,
            'count'       => count($events),
        ]);
    }
